...
class BottleDisplay:
    ...
    def draw(self, surface, sensor_data):
        ...
        draw_text(surface, f"of {sensor_data.daily_target_liters}L daily goal", 
                 (self.center[0], self.center[1] - 10), self.GRAY, 20)

        # Refills needed
        refills_text = "😊" if sensor_data.all_refills_complete else f"{sensor_data.refills_needed - sensor_data.completed_refills}x"
        draw_text(surface, refills_text, 
                 (self.center[0] - 70, self.center[1] + 40), self.GREEN, 24)
        draw_text(surface, "Refills Needed", 
